...
async def delete_session(session_id: str) -> dict[str, bool]:
    ...
    if agent_manager.base_dir:
        ctx_file = agent_manager.base_dir / "workspace" / "llm_context" / f"{session_id}.md"
        ctx_file.unlink(missing_ok=True)
    return {"ok": True}


@router.get("/sessions/{session_id}/llm-context")
async def get_session_llm_context(session_id: str) -> dict[str, str]:
    if agent_manager.base_dir is None:
        raise HTTPException(status_code=503, detail="Agent manager is not initialized")
    path = agent_manager.base_dir / "workspace" / "llm_context" / f"{session_id}.md"
