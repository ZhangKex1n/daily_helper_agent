...
_PLUGINS_ROOT = os.path.dirname(os.path.abspath(__file__))
_PLUGINS_JSON = os.path.join(_PLUGINS_ROOT, "plugins.json")
_PLUGINS_CONFIG_JSON = os.path.join(_PLUGINS_ROOT, "config.json")
_PLUGINS_SOURCE_JSON = os.path.join(_PLUGINS_ROOT, "source.json")
...
class PluginManager:
    ...
    def _load_all_config():
        ...
        """
        all_config_path = _PLUGINS_CONFIG_JSON
        try:
    ...
    def install_plugin(self, repo: str):
        ...
        if not match:
            try:
                with open(_PLUGINS_SOURCE_JSON, "r", encoding="utf-8") as f:
                    source = json.load(f)
        ...
        dirname = os.path.join(_PLUGINS_ROOT, match.group(4))
        try:
