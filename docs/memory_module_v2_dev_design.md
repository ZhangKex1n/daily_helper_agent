...
`api_server/service/session_manager.py` 与样例 `memory_module_v1/sessions/*.json` 显示：
...
建议把它们加入 `api_server/requirements.txt`（或你的依赖管理文件）中。

### 环境变量（建议）

以下变量建议统一由 `api_server/config/.env` 管理（名称可按你的习惯调整）：
...
- **文件路径**：如 `api_server/service/session_manager.py`、`C:\Users\...`、`./foo/bar.ts`
